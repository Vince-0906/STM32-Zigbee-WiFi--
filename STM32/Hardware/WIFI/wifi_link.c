 ...
 * Any step timeout or explicit error falls back to BACKOFF, toggles EN low,
...
static uint8_t      s_wifi_connected_seen = 0;
...
static void remember_rx_line(const char *s)
{
    ...
    if (strstr(s_last_at_line, "+EVENT:WIFI_CONNECT") != 0) {
        s_wifi_connected_seen = 1;
    }
    if (strstr(s_last_at_line, "+EVENT:WIFI_GOT_IP") != 0) {
        s_wifi_got_ip_seen = 1;
    }
...
static void enter(wifi_st_t st, uint32_t now_ms, uint32_t timeout_ms)
{
    ...
    s_last_at_line[0] = '\0';
    if (st == W_WJAP) {
        s_wifi_connected_seen = 0;
        s_wifi_got_ip_seen = 0;
    } else if (st == W_SOCKET) {
